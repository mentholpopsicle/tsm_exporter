...
func dsmadmcOccupancys(target *config.Target, ctx context.Context, logger log.Logger) (string, error) {
	...
	query := fmt.Sprintf("SELECT %s FROM occupancy",strings.Join(queryFields, ","),)
	// Filter by configured storage pools
	...
	query += fmt.Sprintf(" GROUP BY %s",strings.Join(groupFields, ","),)
